...
def _parse_result(path: Path, party: str, profile: AdapterProfile) -> AdapterResult:
    ...
    return AdapterResult(entry_type, body.strip(), refs, appendix.strip(), runtime_model.strip())
...
class BrokerController:
    ...
    def _prepare_case(self, thread: str) -> tuple[dict[str, SourceExport], DocketRevision, datetime]:
        ...
        deadline = self.now + timedelta(seconds=self.config.timing.whole_case_timeout_seconds)
        manifest = {
            **identity,
    ...
    def open_case(
        ...
        *,
        ...
        exports, docket, _ = self._prepare_case(thread)
    ...
    def _invoke(
        ...
        *,
        ...
        remaining = int((deadline - self.now).total_seconds())
        if remaining <= 0:
            raise AdapterError(f"refused: whole-case deadline expired for thread {thread!r}")
        profile = self.config.profiles[party]
        ...
        try:
            ...
            raise AdapterError(
                f"adapter {party!r} timed out after {timeout}s within the whole-case budget",
    ...
    def invoke_and_post(
        ...
        *,
        ...
        entry_id = channel.post(
            ...
            result.entry_type,
            thread,
            result.body + appendix + provenance,
            refs=result.refs,
            name=channel_name,
            _brokered=True,
        )
