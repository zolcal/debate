...
def test_init_creates_config_mailbox_and_fresh_doorbell(root):
...
def test_init_refuses_duplicate_names(tmp_path):
...
def test_post_appends_entry_and_flips_turn(root):
...
def test_out_of_turn_post_is_refused_within_open_thread(root):
...
def test_second_thread_refused_while_one_is_open(root):
    post(root, "alice", "review-request", "feature-x", "please review")
...
def test_close_clears_thread_AND_turn(root):
...
def test_closer_can_open_the_next_thread(root):
...
def test_one_shot_close_leaves_no_open_thread(root):
...
def test_supervisor_posts_do_not_flip_the_turn(root):
    post(root, "alice", "review-request", "feature-x", "please review")
...
def test_supervisor_may_post_out_of_turn(root):
    post(root, "alice", "review-request", "feature-x", "please review")
...
def test_thread_cap_refuses_further_posts_but_allows_close(root):
...
def test_unknown_sender_type_and_slug_are_refused(root):
...
def test_force_allows_supervisor_directed_thread_override(root):
    post(root, "alice", "review-request", "feature-x", "please review")

    entry_id = post(root, "bob", "info", "urgent-note", "out-of-band note", force=True)

    assert entry_id == "MSG-2"


def test_write_then_signal_ordering_survives_partial_read(root):
    ...
    signal = read_signal(root)
    entries = read_entries(root)
    assert [e.seq for e in entries] == list(range(1, int(signal["seq"]) + 1))


def test_multiline_bodies_round_trip(root):
    body = "line one\n\n  indented line\nlast line"
