...
On your next launch in a project, Debate shows a short setup notice if the project is
not ready (generated by a read-only, zero-model-call session-start hook — Codex asks
you to trust the hook first; until you do, automatic onboarding stays off and setup
works by asking for it). Reply **"set up Debate"** and approve, in your agent's own UI,
which locally detected agents this project may seat. Detection is evidence, not
